...
describe("Teste de cadastro de Tipo Parametro", ()=>{
    ...
    test("Cadastrando TipoParametro unidadeRepetida", async()=>{
        const mockRes = (MockResponse.resSemLocals.status(400).send as jest.Mock).mockClear()
        await TipoParametroController.cadastrar(MockCadastro.reqTipoParametroComUnidadeJaExistente, MockResponse.resSemLocals);
        expect(mockRes.mock.calls[0][0]).toBe("unidadeTipoParametro já cadastrada")
    })
})